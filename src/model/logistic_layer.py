...
class LogisticLayer():
    ...
    def forward(self, inp):
        ...
        self.inp = inp
        self.outp = self.activation(np.sum(np.multiply(self.weights, inp[:,None]),axis=0))
        
        

    def computeDerivative(self, nextDerivatives=None, nextWeights=None, labels=None):
        """
        ...
        if self.is_classifier_layer:
            self.deltas = self.outp*(1-self.outp)*(labels-self.outp)
        else:
    ...
    def updateWeights(self, learningRate):
        ...
        self.weights += (self.inp*self.deltas*learningRate).reshape(785,1)
        pass

    def _fire(self, inp):
        sign = Activation.sign(self.activation(np.dot(np.array(inp), self.weights))-0.5)
        return  sign[0]
